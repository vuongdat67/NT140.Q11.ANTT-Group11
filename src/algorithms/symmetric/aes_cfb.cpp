/**
 * @file aes_cfb.cpp
 * @brief AES-CFB encryption implementation
 */
...
AES_CFB::AES_CFB(size_t key_bits) : key_bits_(key_bits) {
    if (key_bits != 128 && key_bits != 192 && key_bits != 256) {
        throw std::invalid_argument("AES-CFB key size must be 128, 192, or 256 bits");
    }
    
    switch (key_bits) {
        case 128:
            type_ = core::AlgorithmType::AES_128_CFB;
            botan_name_ = "AES-128/CFB";
            break;
        case 192:
            type_ = core::AlgorithmType::AES_192_CFB;
            botan_name_ = "AES-192/CFB";
            break;
        case 256:
        default:
            type_ = core::AlgorithmType::AES_256_CFB;
            botan_name_ = "AES-256/CFB";
            break;
    }
    
    spdlog::debug("Created AES-{}-CFB algorithm", key_bits);
}

std::string AES_CFB::name() const {
    return "AES-" + std::to_string(key_bits_) + "-CFB";
}

core::AlgorithmType AES_CFB::type() const {
    return type_;
}

core::CryptoResult AES_CFB::encrypt(
    ...
    try {
        ...
        if (!cipher) {
            result.success = false;
            result.error_message = "Failed to create AES-CFB cipher";
            return result;
        }
        ...
        spdlog::error("AES-CFB encryption failed: {}", e.what());
    }
    
    return result;
}

core::CryptoResult AES_CFB::decrypt(
    ...
    try {
        ...
        if (!config.nonce.has_value() || config.nonce.value().size() != iv_size()) {
            result.success = false;
            result.error_message = "IV must be provided for CFB decryption (16 bytes)";
            return result;
        }
        ...
        result.success = false;
        result.error_message = std::string("Error: ") + e.what();
        spdlog::error("AES-CFB decryption failed: {}", e.what());
    }
    
    return result;
}

bool AES_CFB::is_suitable_for(core::SecurityLevel level) const {
    switch (level) {
