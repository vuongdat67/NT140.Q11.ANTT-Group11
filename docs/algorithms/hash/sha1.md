...
- Kích thước đầu ra: $160\ \text{bit} = 5 \times 32\text{-bit words}$.
- Hằng số khởi tạo: $H_0,\dots,H_4$ (mỗi giá trị 32‑bit).
- Các hàm bitwise:
    - $\operatorname{Ch}(x,y,z) = (x\land y)\oplus(\lnot x\land z)$
    - $\operatorname{Parity}(x,y,z) = x\oplus y\oplus z$
    - $\operatorname{Maj}(x,y,z) = (x\land y)\oplus(x\land z)\oplus(y\land z)$
- Mở rộng từ $W_0..W_{15}\to W_0..W_{79}$:
    - $$W_t = \operatorname{ROTL}_1\bigl(W_{t-3}\oplus W_{t-8}\oplus W_{t-14}\oplus W_{t-16}\bigr)\quad\text{cho }t=16\dots79$$
- Bước nén (cho $t=0\dots79$):
    - $$\mathrm{TEMP} = \bigl(\operatorname{ROTL}_5(a) + f_t(b,c,d) + e + K_t + W_t\bigr)\bmod 2^{32}$$
    - $$e=d,\quad d=c,\quad c=\operatorname{ROTL}_{30}(b),\quad b=a,\quad a=\mathrm{TEMP}$$
    - Trong đó các phép toán bitwise và ROTL là trên 32‑bit, và cộng là modulo $2^{32}$.
- Padding:
    - Thêm một bit `'1'`, rồi các bit `'0'` đến khi độ dài (tính bằng bit) ≡ $448\pmod{512}$, sau đó thêm trường 64‑bit biểu diễn độ dài gốc (theo bit) ở dạng big‑endian.
- Độ an toàn toán học:
    - SHA‑1 hiện đã bị chứng minh dễ dẫn đến va chạm (collision) — không được dùng cho ứng dụng mới cần chống va chạm.
