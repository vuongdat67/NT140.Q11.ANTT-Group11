...
- salt $S$,
- số vòng lặp $c$,
- PRF (thường HMAC-SHA256/512),
- $dklen$ độ dài khóa cần ra.

Định nghĩa:
$DK = T_1 \mathbin\| T_2 \mathbin\| \dots \mathbin\| T_{\lceil dklen/hlen\rceil}$

với
$T_i = F(P,S,c,i)$ và
$F(P,S,c,i) = U_1 \oplus U_2 \oplus \dots \oplus U_c$

$U_1 = PRF(P, S \mathbin\| INT(i))$  
$U_j = PRF(P, U_{j-1})\quad (j>1)$

Trong đó $INT(i)$ là biểu diễn 32-bit big-endian của $i$, $hlen$ là độ dài đầu ra PRF.
