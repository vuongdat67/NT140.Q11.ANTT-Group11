...
- Tham số: $N$ (power‑of‑2 CPU/memory cost), $r$ (block size), $p$ (parallelism) và $dkLen$ (độ dài khóa đầu ra).
- Độ phức tạp bộ nhớ (trên một instance): Memory $\approx 128 \cdot r \cdot N$ bytes.
- Độ phức tạp thời gian (tỉ lệ): Time $\propto N \cdot p$ (các phép xử lý nội bộ cố định phụ thuộc $r$).
- Công thức tổng quát (chi phí): WorkFactor $\approx O(N\cdot r \cdot p)$.
