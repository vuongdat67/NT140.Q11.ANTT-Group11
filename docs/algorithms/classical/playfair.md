...
$$
\text{Gọi vị trí ký tự }x\text{ là }(r_x,c_x),\qquad r_x,c_x\in\{0,1,2,3,4\}.
$$

Với digraph \((a,b)\), ký tự mã hóa \((a',b')\) được cho bởi
$
(a',b')=
\begin{cases}
\big((r_a,\,(c_a+1)\bmod 5),\; (r_b,\,(c_b+1)\bmod 5)\big), & r_a=r_b,\\[6pt]
\big(((r_a+1)\bmod 5,\,c_a),\; ((r_b+1)\bmod 5,\,c_b)\big), & c_a=c_b,\\[6pt]
\big((r_a,\,c_b),\; (r_b,\,c_a)\big), & \text{khác (hình chữ nhật)}.
\end{cases}
$
